...
// Pro/Elite plan (or an admin) sees every team member's jobs and can use the
// date filter / calendar view; a free-plan member only ever sees their own.
async function isPaidOrAdmin(req: Request): Promise<boolean> {
...
// Trimmed, read-only activity feed visible to every team member (not just
// admins) - same underlying data as the admin activity log.
jobsRouter.get("/activity", async (_req, res) => {
  const contacts = await prisma.contact.findMany({
    where: { status: { in: ["sent", "dry_run", "send_failed", "skipped_disabled"] } },
    orderBy: { sentAt: "desc" },
    take: 100,
...
jobsRouter.post("/", async (req, res) => {
  ...
  const job = await prisma.job.create({
    data: { url: parsed.data.url, addedById: req.user!.id, status: "pending" },
  });
...
// Plain members see only jobs they personally added; admins and Pro/Elite
// members see the whole team's jobs. Pro/Elite can additionally filter by
// posted-date range (used by the calendar view) - free plan gets neither.
const dateOnly = /^\d{4}-\d{2}-\d{2}$/;

jobsRouter.get("/", async (req, res) => {
  const seesAll = await isPaidOrAdmin(req);
  const where: Record<string, unknown> = seesAll ? {} : { addedById: req.user!.id };

  const { from, to } = req.query;
  if (typeof from === "string" || typeof to === "string") {
    if (!seesAll) return res.status(403).json({ error: "Date filtering is a Pro/Elite feature" });
...
// Shared by the job-detail and contacts endpoints below - a plain member
// can't reach another member's job by guessing its id any more than they
// could see it in their own job list.
async function canSeeJob(req: Request, addedById: number): Promise<boolean> {
  if (req.user!.id === addedById) return true;
  return isPaidOrAdmin(req);
}
